    ...
    for cidx in range(len(cascontig)):
        ...
        for gidx in range(len(allgene_sorted)):
            ...
            if hmmid:
                ...
                shortdes = "unk"
            if outfile:
                #outfile.write(" ".join([allgene_sorted[gidx][0], sampleid, cascontig[cidx], allgene_sorted[gidx][1], allgene_sorted[gidx][2], allgene_sorted[gidx][3], shortdes, allgene_sorted[gidx][6]])
                outfile.write(1)
            #if ((filtered and allgene_sorted[gidx][6]) and (not isolate[gidx]):
            #    filtered.write(" ".join([allgene_sorted[gidx][0], sampleid, cascontig[cidx], allgene_sorted[gidx][1], allgene_sorted[gidx][2], allgene_sorted[gidx][3], shortdes, allgene_sorted[gidx][6]])
            if filtered and allgene_sorted[gidx][6] and (not isolate[gids]):
                #filtered.write(" ".join([allgene_sorted[gidx][0], sampleid, cascontig[cidx], allgene_sorted[gidx][1], allgene_sorted[gidx][2], allgene_sorted[gidx][3], shortdes, allgene_sorted[gidx][6]])
                filtered.write(2)
    #print(hmmscanfile + " all known-cas " + str(len(casgene_all)) + " unknown-cas " + str(len(casunk_all)) + " isolate-cas-tagged " + str(tagnum))
    print(" ".join([hmmscanfile, "all known-cas", str(len(casgene_all)), "unknown-cas", str(len(casunk_all)), "isolate-cas-tagged", str(tagnum)])
    return casgene_all, casunk_all

def main():
    gadir = "/u/yye/CRISPRpk/CRISPRone/scripts/"
    gafile0 = gadir + "local/cas-db/cas-CRISPR2018.GA"
    crisprfile0 = gadir + "local/cas-db/cas-CRISPR2018.GA"
    #both are GA files
