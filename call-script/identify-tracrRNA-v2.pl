...
`perl $crisprone/call-script/negative-strain.pl $output > $crRNA`;

`$crisprone/bin/blast+/bin/makeblastdb  -in $ARGV[2]  -dbtype 'nucl' -parse_seqids`;

`$crisprone/bin/blast+/bin/blastn -db $ARGV[2] -query $crRNA -out $blast_out -outfmt 6  -word_size 11`;
...
#	    if ($tmp[3]>=$repeat_len*0.6 && $tmp[4]<=5){
		...
		if (($start<$repeat_start || $start>$repeat_end) && ($end<$repeat_start || $end>$repeat_end)){
			...
			$mismatch=$tmp[4];
			$gaps=$tmp[5];
			if (($tmp[6]!=1) || ($tmp[7]!=$repeat_len)){
			    $genome_new_start=$start-($tmp[6]-1);
			    $genome_new_end=$end+($repeat_len-$tmp[7]);
			    $genome_seq=`$crisprone/bin/blast+/bin/blastdbcmd -db $ARGV[2] -dbtype 'nucl' -entry "$tmp[1]" -range "$genome_new_start-$genome_new_end" -strand "$genome_strand" `;
			    chomp $genome_seq;
			    @gen_seq=split(/\n/,$genome_seq);
			    $part_match=0;
			    $part_mismatch=0;
			    @ori_tmp_seqs=split(//,$gen_seq[1]);
			    @repeat_tmp_seqs=split(//,$typeIIrepeat{$tmp[0]});
			    print "$tmp[6]\t$tmp[7]\t$repeat_len\n";
			    print "$typeIIrepeat{$tmp[0]}\n$gen_seq[1]\n";
			    if ($tmp[6]!=1){
				for ($i=1;$i<=($tmp[6]-1);$i++){
				    $real_i=$i-1;
				    print "Begin: $repeat_tmp_seqs[$real_i] vs $ori_tmp_seqs[$real_i]\n";
				    if ($repeat_tmp_seqs[$real_i] ne $ori_tmp_seqs[$real_i]){
					$mismatch++;
				    }
				}
			    }
			    if ($tmp[7]!=$repeat_len){
				for ($i=($tmp[7]-$repeat_len);$i<=-1;$i++){
				    $real_i=$i;
				    print "End: $repeat_tmp_seqs[$real_i] vs $ori_tmp_seqs[$real_i]\n";
				    if ($repeat_tmp_seqs[$real_i] ne $ori_tmp_seqs[$real_i]){
					$mismatch++;
                                    }
			...
			if ($mismatch<=($repeat_len*0.5)){
			    $key=$tmp[1]." ".$start;
			    $match=($repeat_len-$mismatch);
			    $value=$cas9_dir{$tmp[0]}."\t".$rna_dir."\t".$tmp[1]."\t".$match."\t".$mismatch."\t".$genome_new_start."\t".$genome_new_end;
#			    $value=$cas9_dir{$tmp[0]}."\t".$rna_dir."\t".$tmp[1]."\t".$tmp[3]."\t".$tmp[4]."\t".$start."\t".$end;
			    $crRNA_bash{$key}=$value;
			    print NEW "$cas9_dir{$tmp[0]}\t$rna_dir\t$tmp[1]\t$match\t$mismatch\t$genome_new_start\t$genome_new_end\n"; 
